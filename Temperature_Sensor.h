...
public:
  ...
  bool initialize() {
    if (AHT_SENSOR) {
      ...
      ds18b20->requestTemperatures();
      if (ds18b20->getTempC(deviceAddress[0]) == -127) {
        Temp_Sensor_Error = true;
      } else {
        last_Temp_Measure = millis();
        Temp_Sensor_Error = false;
        Actual_Temp = ((ds18b20->getTempC(deviceAddress[0]) * 9 / 5) + 32) - 3.1;  // convert from C to F and offset by 3.1 degrees
      }
    }
    return false;
  }
  ...
  void measureTemperature() {
    if (AHT_SENSOR) {
      ...
      ds18b20->requestTemperatures();
      float tempC = ds18b20->getTempC(deviceAddress[0]);
